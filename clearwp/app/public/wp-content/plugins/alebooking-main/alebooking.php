...
class AleBooking
{
    ...
    public function register(){
        ...
        //Load template
        add_filter('template_include', [$this,'house_template']);
    ...
    public function settings_init(){
        ...
        add_settings_field('title_for_houses', esc_html__('Archive page title','alebooking'), [$this, 'title_for_houses_html'], 'alebooking_settings', 'booking_settings_section');
    ...
    public function title_for_houses_html(){
        $options = get_option('booking_settings_options'); ?>

        <input type="text" name="booking_settings_options[title_for_houses]" value="<?php echo isset($options['title_for_houses']) ? $options['title_for_houses'] : "";  ?>" />
    ...
    //Custom template for houses
    public function house_template($template){

        if(is_post_type_archive('house')){
            $theme_files = ['archive-house.php','alebooking/archive-house.php'];
            $exist = locate_template( $theme_files, false);
            if($exist != ''){
                return $exist;
            } else {
                return plugin_dir_path(__FILE__).'templates/archive-house.php';
            }
        }
        return $template;
    }
    ...
    public function custom_post_type(){
        register_post_type('house',
            ...
            'has_archive' => true,
            'rewrite' => ['slug'=>'houses'],
            'label'  => esc_html__( 'house', 'alebooking' ),
        ...
        $args = array(
            ...
            'query_var'         => true,
            'rewrite'           => array( 'slug' => 'houses/location' ),
        );
        ...
        register_taxonomy('type','house',$args_type);
    }
}
